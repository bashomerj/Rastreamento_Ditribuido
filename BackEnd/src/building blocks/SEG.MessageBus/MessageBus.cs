...
namespace SEG.MessageBus
{
    public class MessageBus : IMessageBus
    {
        ...
        public void Send<T>(string queue, T message) where T : IntegrationEvent
        {
            TryConnect();
            _bus.Send(queue, message);
        }

        public async Task SendAsync<T>(string queue, T message) where T : IntegrationEvent
        {
            TryConnect();
            await _bus.SendAsync(queue, message);
        }

        public void Receive<T>(string queue, Action<T> message) where T : class
        {
            TryConnect();
            _bus.Receive(queue, message);
        }
